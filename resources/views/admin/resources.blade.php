    ...
    <table class="striped">
        @foreach($resources as $resource)
            <tr>
                ...
                <td>
                    @if($resource->webinar)
                        Attached Webinar: {{$resource->webinar->name}}
                    @elseif(!$resource->course)
                        No attached webinar / course
                    @endif
                    @if($resource->course)
                        Attached Course: {{$resource->course->name}}
                    @endif
                </td>
                @if($resource->expiry)
                ...
                @else
                    <td>No expiry</td>
                @endif
                <td><a href="/admin/resources/edit/{{$resource->id}}"><i class="material-icons">edit</i></a></td>
                <td>
                    <form action="{{route('admin.resources-delete', $resource->id)}}" method="post">{{csrf_field()}}</form>
...
@section('footer')
    <div class="fixed-action-btn">
        <a class="btn-floating waves-effect btn-large" href="{{route('admin.resources-create')}}">
